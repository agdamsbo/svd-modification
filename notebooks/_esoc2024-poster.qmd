...
ls <- c(FALSE, TRUE) |>
  ...
  (\(.x){
    .x |> dplyr::mutate(pase_0 = pase_0>median(pase_0,na.rm=TRUE))
  })()|>
    dplyr::mutate(
...
ls |> gtsummary::tbl_merge()

p1 <- ls |> gtsummary::tbl_merge() |> 
  coef_forrest_plot(cols = viridis::viridis(7)[c(1,5)],x.tics = create_log_tics(c(7, .4, .6, .8, 1)) )
